...
async function getOracleStorageRows(): Promise<{ rows: StorageConfigEntry[]; liveTypes: Set<string> }> {
  ...
  try {
    ...
    // Substring match, not exact equality — see database_pipeline.ts's
    // OracleAutonomousAdapter for why (Oracle's feed has confirmed whitespace/
    // formatting drift between rows for the same conceptual line item).
    const blockRate = findPrice(catalog, item => nameIncludes(item, 'storage - block volume - storage'));
    const fileRate = findPrice(catalog, item => nameIncludes(item, 'file storage - storage'));
...
export class StoragePricingPipeline extends PricingPipeline {
  async run() {
    ...
    try {
      ...
      const inserted = await this.saveRecords(records, 'storage');
      results.push({
        provider: 'aws',
        service: 'Storage',
        status: 'success',

...
// `records[0]?.dataSource` silently mislabeled the run summary by whichever
// row sorted first (e.g. Azure Functions always reporting 'static_config'
// once the static Container Apps rows were appended after a successful live
// fetch). Per-row data was always correct; only this summary signal lied.
// Confirmed 2026-07-29 — same bug found in containers_pipeline.ts.
function summarizeDataSource(records: PricingRecord[]): string {
  const sources = new Set(records.map(r => r.dataSource));
  if (sources.size === 0) return 'static_config';
  if (sources.size === 1) return [...sources][0] as string;
  return 'mixed';
}

// DEAD CODE — confirmed 2026-07-29: not instantiated anywhere in this file's actual pipeline
// run() (that calls AWSLambdaLiveAdapter from serverless_adapters_live.ts directly). Wraps
// AwsLambdaScraper, which is itself dead/unreconciled code — see aws_lambda.ts's header
// comment before wiring this back in.
...
export class ServerlessPricingPipeline extends PricingPipeline {
  ...
  async run() {
    ...
    try {
      ...
      const inserted = await this.saveRecords(records, 'serverless');
      results.push({
        provider: 'aws',
        service: 'Lambda',
        status: 'success',
        count: inserted,
        dataSource: summarizeDataSource(records)
      });
    } catch (error: any) {

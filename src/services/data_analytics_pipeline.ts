  ...
  async run(): Promise<{ provider: string; status: string; count?: number; message?: string }[]> {
    ...
    for (const adapter of this.adapters) {
      try {
        ...
        // Sum what each per-service group actually inserted, rather than the
        // fetched record count — see PricingPipeline.saveRecords().
        let inserted = 0;
        for (const group of byService.values()) {
          inserted += await this.saveRecords(group, 'data_warehouse');
        }

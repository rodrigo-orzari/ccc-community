  ...
  async run() {
    ...
    for (const p of STATIC_PROVIDERS) {
      ...
      try {
        ...
        const records: PricingRecord[] = p.rows.map(inst => {
          ...
          if (p.slug === 'azure') {
            if (inst.type === 'Service Bus Standard' && liveSb?.standardOpPrice != null) {
              ...
              // 730 hours/month (365*24/12), the average-month convention used
              // everywhere else in this codebase for hourly→monthly conversion
              // (confirmed against the static fallback: $668/mo static implies
              // ~$0.9155/hr * 730 ≈ $668, not * 720). Using 24*30=720 here
              // underpriced Service Bus Premium's live figure by ~1.4% vs every
              // other hourly-billed service on the site. Confirmed 2026-07-29.
              price = Math.round(liveSb.premiumHourPrice * 730);
              dataSource = 'live_api';
              isLiveFetched = true;
        ...
        const inserted = await this.saveRecords(records, 'integration');
        results.push({
          provider: p.slug,
          service: 'Integration',
          status: 'success',

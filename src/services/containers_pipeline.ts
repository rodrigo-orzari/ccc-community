...
// `records[0]?.dataSource` silently mislabeled the whole run's summary by
// whichever row happened to sort first — e.g. AWS/DigitalOcean Containers
// always reported 'static_config' even when the live Fargate/DOKS fetch fully
// succeeded, because a static-only row (EKS/App Platform nodes appended
// alongside live rows) happened to be first. Per-row data in the DB was
// always correct; only this run-summary signal lied. Confirmed 2026-07-29.
  ...
  async run() {
    ...
    for (const provider of providers) {
      try {
        ...
        const inserted = await this.saveRecords(records, 'containers');
        results.push({

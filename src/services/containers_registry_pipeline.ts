...
export class ContainersRegistryPricingPipeline extends PricingPipeline {
  ...
  async run(): Promise<{ provider: string; status: string; count?: number; message?: string }[]> {
    ...
    try {
      ...
      const inserted = await this.saveRecords(records, 'registry');
      
      console.log(`✅ AWS ECR: ${inserted} pricing tiers configured`);
      results.push({ provider: 'aws', status: 'success', count: inserted });
    } catch (err) {
    ...
    try {
      ...
      const inserted = await this.saveRecords(records, 'registry');
      console.log(`✅ Azure ACR: ${inserted} pricing tiers configured`);
      results.push({ provider: 'azure', status: 'success', count: inserted });
    } catch (err) {
    ...
    try {
      ...
      const inserted = await this.saveRecords(records, 'registry');
      console.log(`✅ Google Artifact Registry: ${inserted} pricing tiers configured`);
      results.push({ provider: 'gcp', status: 'success', count: inserted });
    } catch (err) {
    ...
    try {
      ...
      const inserted = await this.saveRecords(records, 'registry');
      console.log(`✅ Oracle Container Registry: ${inserted} pricing tiers configured`);
      results.push({ provider: 'oracle', status: 'success', count: inserted });
    } catch (err) {
    ...
    try {
      ...
      const inserted = await this.saveRecords(records, 'registry');
      console.log(`✅ DigitalOcean Container Registry: ${inserted} pricing tiers configured`);
      results.push({ provider: 'digitalocean', status: 'success', count: inserted });
    } catch (err) {
    ...
    try {
      ...
      const inserted = await this.saveRecords(records, 'registry');
      console.log(`✅ Alibaba Container Registry: ${inserted} pricing tiers configured`);
      results.push({ provider: 'alibaba', status: 'success', count: inserted });
    } catch (err) {
